...
type FlightPriority = "convenient-time" | "flexibility" | "comfort" | "fastest" | "price" | null;

export default function QuizFlightPriorityPage() {
  const router = useRouter();
  const { responses, saveResponse } = useQuiz();
  const [selected, setSelected] = useState<FlightPriority>(null);

  // Load existing response
  useEffect(() => {
    if (responses.flightPriority) {
      setSelected(responses.flightPriority as FlightPriority);
    } else {
      // Selecionar primeira opção por padrão
      setSelected("convenient-time");
    }
  }, [responses]);

  const handleSelect = (priority: FlightPriority) => {
  ...
  const handleContinue = async () => {
    ...
    await saveResponse("flightPriority", selected);
    
    // Redirect to next question
    router.push("/quiz/business/work-intensity");
  };

  const options = [
    {
      id: "convenient-time" as FlightPriority,
      label: "Horário conveniente",
      sublabel: "(evitar madrugada)",
      iconSrc: "/icons/iconsquiznegocios/icon página prioridade em voos alternativa horário conveniente.svg"
    },
    {
      id: "flexibility" as FlightPriority,
      label: "Flexibilidade de mudança",
      iconSrc: "/icons/iconsquiznegocios/icon página prioridade em voos alternativa flexibilidade de mudança.svg"
    },
    {
      id: "comfort" as FlightPriority,
      label: "Conforto (executiva)",
      iconSrc: "/icons/iconsquiznegocios/icon página prioridade em voos alternativa conforto.svg"
    },
    {
      id: "fastest" as FlightPriority,
      label: "Tempo (voo mais rápido)",
      iconSrc: "/icons/iconsquiznegocios/icon prioridade em voos alternativa tempo.svg"
    },
    {
      id: "price" as FlightPriority,
      label: "Preço (dentro do budget)",
      iconSrc: "/icons/iconsquiznegocios/icon página prioridade em voos alternativa preço dentro do budget.svg"
    }
  ];

  return (
    <div className="min-h-screen bg-[#F1F1F1] flex flex-col gap-[36px] py-[49px] px-[10px]">
      {/* Container Principal */}
      <div className="w-full max-w-[375px] mx-auto flex flex-col items-center gap-[42px] px-4 pb-20">
        ...
        <div className="w-full flex flex-col items-center gap-[37px] px-[38px] py-[13px]">
          {/* Ícone de Aeroporto/Avião */}
          <div className="w-[95px] h-[95px] flex items-center justify-center">
            <Image
              src="/icons/iconsquiznegocios/icon superior pagina prioridade em voos.svg"
              alt="Prioridade em voos"
              width={95}
              height={95}
