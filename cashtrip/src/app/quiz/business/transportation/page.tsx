...
type Transportation = "car_rental" | "uber_taxi" | "private_transfer" | "public_transport" | null;

export default function QuizTransportationPage() {
  const router = useRouter();
  const { responses, saveResponse } = useQuiz();
  const [selected, setSelected] = useState<Transportation>(null);

  // Load existing response
  useEffect(() => {
    if (responses.transportation) {
      setSelected(responses.transportation as Transportation);
    } else {
      // Selecionar primeira opção por padrão
      setSelected("car_rental");
    }
  }, [responses]);

  const handleSelect = (transportation: Transportation) => {
    setSelected(transportation);
  };
  ...
  const handleContinue = async () => {
    ...
    await saveResponse("transportation", selected);
    
    // Redirect to next question
    router.push("/quiz/business/dinner");
  };

  const options = [
    {
      id: "car_rental" as Transportation,
      label: "Carro alugado",
      sublabel: "(liberdade/reuniões)",
      iconSrc: "/icons/iconsquiznegocios/icon página locomoção no destino alternativa carro alugado.svg"
    },
    {
      id: "uber_taxi" as Transportation,
      label: "Uber/Taxi",
      sublabel: "(praticidade)",
      iconSrc: "/icons/iconsquiznegocios/icon página locomoção no destino alternativa uber taxi.svg"
    },
    {
      id: "private_transfer" as Transportation,
      label: "Transfer privado",
      sublabel: "(empresa paga)",
      iconSrc: "/icons/iconsquiznegocios/icon página locomoção no destino alternativa transfer privado.svg"
    },
    {
      id: "public_transport" as Transportation,
      label: "Transporte público",
      sublabel: "(eficiente)",
      iconSrc: "/icons/iconsquiznegocios/icon página locomoção no destino alternativa transporte público.svg"
    }
  ];
    ...
    <div className="min-h-screen bg-[#F1F1F1] flex flex-col gap-[27px] py-[49px] px-[10px]">
      ...
      <div className="w-full max-w-[375px] mx-auto flex flex-col items-center gap-[42px] px-4 pb-20">
        ...
        <div className="w-full flex flex-col items-center gap-[37px] px-[38px] py-[13px]">
          {/* Ícone de Carro/Automóvel */}
          <div className="w-[95px] h-[95px] flex items-center justify-center">
            <Image
              src="/icons/iconsquiznegocios/Icon superior página locomoção no destini negócios quiz.svg"
