...
type Dinner = "frequent" | "occasional" | "none" | null;

export default function QuizDinnerPage() {
  const router = useRouter();
  const { responses, saveResponse } = useQuiz();
  const [selected, setSelected] = useState<Dinner>(null);

  // Load existing response
  useEffect(() => {
    if (responses.dinner) {
      setSelected(responses.dinner as Dinner);
    } else {
      // Selecionar primeira opção por padrão
      setSelected("frequent");
    }
  }, [responses]);

  const handleSelect = (dinner: Dinner) => {
    setSelected(dinner);
  };
  ...
  const handleContinue = async () => {
    ...
    await saveResponse("dinner", selected);
    
    // Redirect to next question
    router.push("/quiz/business/post-work");
  };

  const options = [
    {
      id: "frequent" as Dinner,
      label: "Sim, frequentemente",
      sublabel: "(preciso sugestões de restaurantes business)",
      iconSrc: "/icons/iconsquiznegocios/icon página jantar de negócios alternativa sim, frequentemente.svg"
    },
    {
      id: "occasional" as Dinner,
      label: "Ocasional",
      sublabel: "(1-2 jantares especiais)",
      iconSrc: "/icons/iconsquiznegocios/icon página janter de negócios alternativa ocasional.svg"
    },
    {
      id: "none" as Dinner,
      label: "Não",
      sublabel: "(refeições no hotel/rápidas)",
      iconSrc: "/icons/iconsquiznegocios/icon página jantar de negócios alternativa não(refeições no hotel).svg"
    }
  ];
    ...
    <div className="min-h-screen bg-[#F1F1F1] flex flex-col gap-[27px] py-[49px] px-[10px]">
      ...
      <div className="w-full max-w-[375px] mx-auto flex flex-col items-center gap-[42px] px-4 pb-20">
        ...
        <div className="w-full flex flex-col items-center gap-[37px] px-[38px] py-[13px]">
          {/* Ícone de Jantar/Restaurante */}
          <div className="w-[95px] h-[95px] flex items-center justify-center">
            <Image
              src="/icons/iconsquiznegocios/icon superior página jantar de negócios.svg"
              alt="Jantar de negócios"
              width={95}
              height={95}
